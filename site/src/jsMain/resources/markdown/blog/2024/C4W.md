...
size down in the future, but it's hard to imagine they will ever significantly improve past Flutter's current size, if
...
It is possible for early product prototypes that a shared UI framework can help a small team reach a large number of
users very quickly. However, in general, as a product matures, its company will hire multiple teams to handle separate
platforms.

Each team can move a lot faster if they are allowed to progress independently, including making their own UI decisions.
...
In retrospect, it would probably have been far less friction to let each team build their own UIs which then got reviewed
by a central committee whose job was to verify shared UI guidelines.
...
library in this space. Both are battle tested across so many websites.

If you need it, there are many JavaScript graphics libraries out there that you can use to create stunning visual
effects, charts, and images. In fact, Kobweb provides an OpenGL demo (run `kobweb create examples/opengl` to clone it)
that uses [WebGL](https://developer.mozilla.org/en-US/docs/Web/API/WebGL_API) to render a rotating cube.
...
Perhaps over time, the suite of Compose Multiplatform libraries with web support will grow, but even then, the
JavaScript ecosystem has a massive head start, and it will only continue to grow at the same time.
...
much useful information about your actual content out of the opaque canvas.
...
finish downloading and run, although this is generally invisible to the user since the script will simply recompute what
is already being shown.

The export process is only possible thanks to Compose HTML, since it works with the DOM. With C4W, there is no way to
populate an initial render instantly, because the HTML will always ever be a blank canvas. Users will simply have to
wait for everything to finish downloading and the JavaScript to run before the page is populated.
...
It turns out that, for security purposes, information about what links a user has visited is only known to the
browser itself and not shared with the developer's script. This is smart, as otherwise a malicious site could query
...
specify visited and unvisited colors, which the browser will then apply to links in a way that is invisible to the
developer.

In short, C4W is not able to render visited links.

Maybe this isn't a big deal to your site -- some sites intentionally disable visited link colors, for artistic purposes.
...
With Compose HTML, you simply create two text inputs with their autocomplete fields set (one to `username` and another
to `currentPassword`) and they will automatically be hooked up.

As C4W uses its own password text field concepts, the browser will be unable to hook it up.
...
Note that I've heard developers mentioning that they encountered major issues when trying out C4W, as it is a framework
that is still in an alpha state. Additionally, [Wasm](https://webassembly.org/) with Garbage Collection is currently in
its infancy and, even at the time of writing this article, is not supported by Safari, which alone has been a
...
Jetpack Compose offers a more modern way to build UIs, building on lessons learned from the past and providing very
succinct and expressive layout primitives. Once you get your head around the basics, it is very intuitive to use and
includes some incredible animation APIs.
...
your application idea, Compose Multiplatform is a great solution for the application prototype.

Or maybe you are creating internal tools at your company. Writing a web app instead of a desktop application helps
you ensure that everyone will always be up to date with latest.
...
Although choosing Compose HTML does imply that you are committing to maintaining a separate UI codebase for your
website, this does not mean that I am rejecting the idea of sharing code entirely.
...
Compose HTML for your website, at which point you can share a ton of common utility code between them.
